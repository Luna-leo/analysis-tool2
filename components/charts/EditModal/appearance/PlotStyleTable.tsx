...
export function PlotStyleTable({ editingChart, setEditingChart, selectedDataSourceItems }: PlotStyleTableProps) {
  ...
  // Function to render the table rows based on mode
  const renderTableRows = () => {
    ...
    if (appearanceMode === "datasource") {
      ...
      return selectedDataSourceItems.map((dataSource, dataSourceIndex) => {
        const param = editingChart.yAxisParams?.[0] // Use first parameter's style for all data sources
        ...
        return (
          <TableRow key={dataSource.id}>
            ...
            <TableCell className="text-xs">
              {renderLineSettings(param, 0, dataSourceIndex)}
            </TableCell>
          </TableRow>
        )
      })
      ...
      return editingChart.yAxisParams.map((param, paramIndex) => (
        <TableRow key={paramIndex}>
          ...
          <TableCell className="text-xs">
            {renderLineSettings(param, paramIndex)}
          </TableCell>
        </TableRow>
      ))
    } else {
      // By Data Source x Parameter mode - one row per combination
      return selectedDataSourceItems.flatMap((dataSource, dataSourceIndex) =>
        editingChart.yAxisParams?.map((param, paramIndex) => {
          ...
          return (
            <TableRow key={key}>
              ...
              <TableCell className="text-xs">
                {renderLineSettings(param, paramIndex, dataSourceIndex)}
              </TableCell>
            </TableRow>
          )
        }) || []
  ...
  // Function to render marker settings
  const renderMarkerSettings = (param: any, paramIndex: number, dataSourceIndex?: number) => {
    return (
  ...
  // Function to render line settings
  const renderLineSettings = (param: any, paramIndex: number, dataSourceIndex?: number) => {
    return (
      <div className="flex items-center gap-1">
        <Popover>
          <PopoverTrigger asChild>
